...
function Ad() {
  ...
  const fetchAd = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${adsPath}/ads/serve`, { headers })
      if (!res.ok) {
  ...
  return (
    <div className="flex flex-row justify-center py-4 advertisement-wrapper">
      <a href={`${adsPath}/ads/${data.id}/click`} aria-label={data.name}>
        <picture className="advertisement-banner">
          <source srcSet={`${adsPath}/banners/${data.path}`} type="image/jpeg" />
          <img src={`${adsPath}/banners/${data.path}`} alt={data.name} />
        </picture>
      </a>
    </div>
  )
