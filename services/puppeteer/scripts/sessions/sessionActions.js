...
const goToAboutUsPage = async (session) => {
  const base = (config.storedogUrl || '').replace(/\/$/, '');
  await session.page.goto(`${base}/about-us`, { waitUntil: 'domcontentloaded', timeout: 15000 });
  const pageTitle = await session.page.title();
  session.log(`"${pageTitle}" loaded`);

  // The LCP image src is filled in after about-us-api returns.
  await session.page.waitForFunction(() => {
    const img = document.querySelector('#about-us-image');
...
const goToFooterPage = async (session) => {
  ...
  try {
    ...
    for (const link of footerLinks) {
      ...
      const isInternal = href && href.startsWith('/') && !href.startsWith('//') && target !== '_blank';
      if (isInternal) {
    ...
    if (internalLinks.length > 0) {
      ...
      const linkText = await internalLinks[randomIndex].evaluate(el => el.textContent?.trim());
      session.log(`Clicking footer link: "${linkText}"`);
      
      const href = await internalLinks[randomIndex].evaluate(el => el.getAttribute('href'));
      ...
      if (isNavigationLink) {
        try {
          await Promise.all([
            session.page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 10000 }),
            internalLinks[randomIndex].click()
          ]);
          ...
          await internalLinks[randomIndex].click();
          await setTimeout(1000);
        }
      } else {
